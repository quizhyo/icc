...
from .data_utils import developer_info, developer_info_static
...
def prediction_model_pipeline(DF, API_KEY, GPT_MODEL):
    ...
    # Kiểm tra DF trước khi gán vào session_state
    if DF is not None:
        if 'data_origin' not in st.session_state:
            st.session_state.data_origin = DF
        st.dataframe(st.session_state.data_origin.describe(), width=1200)
    else:
        st.error("No data available for analysis. Please check your input data.")
        return
    st.dataframe(st.session_state.data_origin.describe(), width=1200)
